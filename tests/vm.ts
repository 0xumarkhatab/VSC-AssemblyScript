...
export async function instantiateStreaming(source, imports) {
  let state = new Asyncify();
  let result = await loader.instantiateStreaming(
    source,
...
export class VmContainer {
  ...
  async call(args: { action: string; payload: string }) {
  ...
  async init() {
    ...
    const partPath = Path.join(__dirname, "vm-runner.js").replace(
      "src",
      "dist"
    );
    ...
    try {
      ...
      const insta = await loader.instantiate(binaryData, {
        env: {
          ...
          abort(msg, file, line, colm) {
            console.log(
              (insta.exports as any).__getString(msg),
              "LN1",
              (insta.exports as any).__getString(file),
              line,
              colm
            );
        ...
        Date: {
          getISODate: (timestamp) => {
            ...
            return insta.exports.__newString(
              new Date(parseInt(epochTime)).toISOString()
            );
          },
          getEpochTime: (dateString) => {
            ...
            const unixEpochTimeInSeconds: number =
              new Date(_datestring).getTime() / 1000;
        ...
        bitcoin: {
          Uint8ArrayFromBufferHex: (hex: number) => {
            function makeAscArray(instance: any, arr: Uint8Array) {
              try {
                let arrptr = instance.exports.__newArrayBuffer(
                  new ArrayBuffer(arr.length)
                );

                for (let index = 0; index < arr.length; index++) {
                  const element = arr[index];
                  (instance.instance.exports as any).setU8(
                    arrptr,
                    index,
                    element
                  );
          ...
          extractPrevBlockLE: (_decodeHex) => {
            ...
            const prevBlock = reverseBytes(
              BTCUtils.extractPrevBlockLE(decodeHex)
            );
          ...
          retarget: (
            ...
            let val = BTCUtils.retargetAlgorithm(
              previousTarget,
              first_timestamp,
              last_timestamp
            );
          ...
          extractTimestampStr: (_decodeHex) => {
            ...
            const timestamp = parseInt(
              reverseBytes(BTCUtils.extractTimestampLE(decodeHex)),
              16
            );
          ...
          extractMerkleRootLE: (_decodeHex) => {
            ...
            const merkleRoot = reverseBytes(
              BTCUtils.extractMerkleRootLE(decodeHex)
            );
          ...
          hash256: (_decodeHex) => {
            let decodeHex: ArrayBuffer =
              insta.exports.__getArrayBuffer(_decodeHex);
            // Example ArrayBuffer
            ...
            const headerHash = reverseBytes(
              BTCUtils.hash256(uint8Array_buffer)
            );
          ...
          validateHeaderChain: (_decodeHex) => {
            ...
            const diff = parseInt(
              ValidateSPV.validateHeaderChain(uint8Array_buffer)
            );
          ...
          isValidProof: (proof) => {
            return ValidateSPV.validateProof(
              deserializeSPVProof(JSON.stringify(proof))
            );
          },
        },
        sdk: {
          ...
          "db.getObject": (keyPtr) => {
            ...
            let value: string;
            if (stateCache.has(key)) value = stateCache.get(key);
            else value = "{}";
      ...
      try {
        ...
        if (ex.name === "RuntimeError" && ex.message === "unreachable") {
          console.log(
            `RuntimeError: unreachable ${JSON.stringify(error)}`,
            error
          );
          return {
            type: "execute-stop",
